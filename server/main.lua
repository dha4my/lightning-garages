...
QBCore.Functions.CreateCallback("lightning-garage:server:GetOutsideVehicle", function(source, cb, plate)
...
QBCore.Functions.CreateCallback("lightning-garages:server:GetVehicleLocation", function(source, cb, plate)
    plate = string.upper(plate)
    local src = source
...
QBCore.Functions.CreateCallback("lightning-garage:server:CheckSpawnedVehicle", function(source, cb, plate)
    plate = string.upper(plate)
    cb(VehicleSpawnerVehicles[plate] ~= nil and VehicleSpawnerVehicles[plate])
end)

RegisterNetEvent("lightning-garage:server:UpdateSpawnedVehicle", function(plate, value)
    plate = string.upper(plate)
    VehicleSpawnerVehicles[plate] = value
end)

QBCore.Functions.CreateCallback('lightning-garage:server:spawnvehicle', function (source, cb, vehInfo, coords, heading, warp)
    ...
    QBCore.Functions.TriggerClientCallback('lightning-garages:client:GetVehicleType', source, function (vehicleType)
...
QBCore.Functions.CreateCallback("lightning-garage:server:GetGarageVehicles", function(source, cb, garage, garageType, category)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
        ...
        GetDepotVehicles(pData.PlayerData.citizenid, 0, garage, function(result)
            ...
            if result[1] then
                ...
                for _, vehicle in pairs(result) do
                    ...
                    if (veh and (GetVehicleBodyHealth(veh) > 1 and GetEntityHealth(veh) > 1)) then
...
QBCore.Functions.CreateCallback("lightning-garage:server:checkOwnership", function(source, cb, plate, garageType, garage, gang)
    plate = string.upper(plate)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback("lightning-garage:server:GetVehicleProperties", function(source, cb, plate)
    local properties = {}
    local result = MySQL.query.await('SELECT mods FROM player_vehicles WHERE plate = ?', {plate})
    if result[1] then
...
RegisterNetEvent('lightning-garage:server:updateVehicle', function(state, fuel, engine, body, properties, plate, garage, location, damage)
    plate = string.upper(plate)
...
RegisterNetEvent('lightning-garage:server:updateVehicleState', function(state, plate, garage)
    plate = string.upper(plate)
    MySQL.update('UPDATE player_vehicles SET state = ?, garage = ?, depotprice = ? WHERE plate = ?',{state, garage, 0, plate})
end)

RegisterNetEvent('lightning-garages:server:UpdateOutsideVehicles', function(Vehicles)
    local src = source
...
QBCore.Functions.CreateCallback("lightning-garage:server:GetOutsideVehicles", function(source, cb)
...
AddEventHandler('onResourceStart', function(resource)
    if resource == GetCurrentResourceName() then
        Wait(100)
        if Config.ResetCars then
            MySQL.update('UPDATE player_vehicles SET state = 1 WHERE state = 0', {})
        end
    end
end)

RegisterNetEvent('lightning-garage:server:PayDepotPrice', function(data)
    local src = source
...
RegisterNetEvent('lightning-garages:server:parkVehicle', function(plate)
    plate = string.upper(plate)
...
QBCore.Functions.CreateCallback("lightning-garage:server:checkVehicleOwner", function(source, cb, plate)
    plate = string.upper(plate)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
...
--Call from qb-phone
QBCore.Functions.CreateCallback('lightning-garages:server:GetPlayerVehicles', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    local Vehicles = {}
...
if Config.EnableTrackVehicleByPlateCommand then
    ...
    TriggerClientEvent('lightning-garages:client:TrackVehicleByPlate', source, args[1])
